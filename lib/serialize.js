const {
  ...
  { isUrl } = require("./index"),
  { toAudio, convert, convert2 } = require("./convert"),
  cmd = {
    1: [
      "-fs 1M",
      "-vcodec",
...
async function serialize(msg, conn) {
  ...
  conn.sendVideo = async (jid, url, quoted, option = {}) => {
    ...
    return conn.sendMessage(
      jid,
      { video: url, mimetype: ext.mimetype },
      { quoted }
    );
  };
  ...
  if (msg.message) {
    ...
    try {
      ...
      msg.quoted.isSelf =
        msg.quoted.participant === conn.decodeJid(conn.user.id);
