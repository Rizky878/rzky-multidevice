const { proto, jidDecode, downloadContentFromMessage, getContentType } = require("@adiwajshing/baileys"),
	...
	country = require("country-language"),
	customLanguage = JSON.parse(fs.readFileSync("./database/language.json")),
...
async function serialize(msg, conn) {
	...
	conn.sendMessage = async (jid, content, options = {}) => {
		...
		const iso639_1 = await phonenumber("+" + msg.sender.split("@")[0]).getRegionCode();
	...
	conn.sendImage = async (jid, url, quoted, option = {}) => {
		...
		url =
			ext.ext !== "webp"
				...
				: buf;
		return conn.sendMessage(jid, { image: url, caption: option.caption }, { quoted });
	};
	...
	conn.sendVideo = async (jid, url, quoted, option = {}) => {
		...
		url =
			ext.ext !== "mp4"
				...
				: buf;
		return conn.sendMessage(jid, { video: url, caption: option.caption, mimetype: ext.mimetype }, { quoted });
	};
	...
	if (msg.message) {
		...
		msg.reply = (text, opt = { isTranslate: true }) =>
			conn.sendMessage(
				msg.from,
				{
					text,
				...
				{ isTranslate: opt.isTranslate, quoted: msg }
			);
