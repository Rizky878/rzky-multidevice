...
module.exports = async (conn, msg) => {
	...
	let ppimg;
	try {
		ppimg = await conn.profilePicturrUrl(participant, "image");
	} catch {
		ppimg = config.thumb;
	}
		...
		if (msg.action == "add" && dataWelcome.id.includes(msg.id)) {
				...
				caption:
					...
					`\n\n*Last Modified:* ${
						dataWelcome.lastUpdate
							? require("moment")(dataWelcome.lastUpdte).format("dddd, DD MMMM YYYY HH:mm:ss")
							: ""
					}`,
			});
			...
			await conn.sendMessage(msg.id, {
				...
				caption:
					...
					`\n\n*Last Modified:* ${
						dataLeft.lastUpdate
							? require("moment")(dataLeft.lastUpdte).format("dddd, DD MMMM YYYY HH:mm:ss")
							: ""
					}`,
			});
		}
