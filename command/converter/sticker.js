...
module.exports = {
	name: "sticker",
	alias: ["stick", "stik", "stiker", "take", "swm", "stickerwm", "wm", "stickergif", "stikergif", "gifstiker", "gifsticker"],
	category: "converter",
	...
	async run(msg, conn) {
		...
		q = q.split("|")
		const packInfo = { "packname" : q[0] ? q[0] : config.packInfo.packname, "author": q[1] ? q[1] : config.packInfo.author }
		...
		if (isQStic) {
				...
				isQDoc &&
				(/image/.test(quoted.message.documentMessage.mimetype) || (/video/.test(quoted.message.documentMessage.mimetype) && quoted.message.documentMessage.fileLength < 2 << 20))
			) {
